...
bc.onmessage = evt => {
    ...
    switch (msg.type) {
        case 'connect': {
            ...
            send({ type: 'users', dst: msg.src, users, rooms: Object.keys(rooms) });
            break;
        }
        case 'disconnect': {
            if (msg.room) {
        ...
        case 'join': {
            if (!msg.room)
                return;
            if (!rooms[msg.room].includes(msg.src))
                rooms[msg.room].push(msg.src);
...
function leaveRoom(room, roomMember) {
    if (!rooms[room])
        return;
...
function generateUnusedValue(prefix, target) {
